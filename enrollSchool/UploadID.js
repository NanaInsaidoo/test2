...
export default function ImageUpload(props) {
  ...
  const { label, name,  deleteID, uploadID } = props


  
  if (uploadID !== "No ID uploaded") {

    return ( 
      <div className={classes.uplaodOuterDiv}>
      <div className={classes.uplaod}>
  
        <label className={classes.label} htmlFor={name}> <Button name = {name} id={name} variant="contained" component="span" className={classes.margin} onClick ={deleteID} >
           Delete ID
      </Button></label>{uploadID}
